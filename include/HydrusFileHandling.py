...
header_and_mime = [
    ...
    # PSB, which is basically PSD v2 and does giganto resolution
    (0, b'8BPS\x00\x02', HC.APPLICATION_PSD),
...
def GenerateThumbnailBytes(path, target_resolution, mime, duration, num_frames, percentage_in=35):
    ...
    if mime in (HC.IMAGE_JPEG, HC.IMAGE_PNG, HC.IMAGE_GIF, HC.IMAGE_WEBP, HC.IMAGE_TIFF, HC.IMAGE_ICON):  # not apng atm
        ...
        if mime == HC.APPLICATION_FLASH:
        ...
        elif mime == HC.APPLICATION_PDF:  # Adding PDF thumbnail support
            ...
            numpy_image = HydrusImageHandling.ResizeNumPyImage(
                numpy_image, target_resolution)  # just in case ffmpeg doesn't deliver right
...
def GetFileInfo(path, mime=None, ok_to_look_for_hydrus_updates=False):
    ...
    if mime in (HC.IMAGE_JPEG, HC.IMAGE_PNG, HC.IMAGE_GIF, HC.IMAGE_WEBP, HC.IMAGE_TIFF, HC.IMAGE_ICON):
        ...
        # this now give None until a better solution can be found
        ...
        (file_duration_in_s, stream_duration_in_s) = HydrusVideoHandling.ParseFFMPEGDuration(
            ffmpeg_lines)
